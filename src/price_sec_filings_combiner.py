...
def get_event_trading_day(filing_date, acceptance_time, trading_days):
    """
...
# FEATURE EXTRACTION
# ============================================================
def compute_features(price_df, event_day, lookback=60):
    """
...
# LABEL GENERATION
# ============================================================
def compute_labels(price_df, event_day, horizons, threshold=0.005):
    """
...
# MAIN PIPELINE
# ============================================================
def build_event_dataset(prices_path, filings_path, output_path):
    ...
    for _, row in tqdm(filings.iterrows(), total=len(filings), desc="Aligning filings"):
        ...
        event_day = get_event_trading_day(filing_date, acceptance_time, trading_days)

        if event_day is None:
            continue

        # Extract features from past prices
        features = compute_features(price_df, event_day, lookback=LOOKBACK_DAYS)
        if features is None:
            continue

        # Compute labels (future returns + classification)
        labels = compute_labels(price_df, event_day, LABEL_HORIZONS, threshold=CLASS_THRESHOLD)
        if labels is None:
            continue
