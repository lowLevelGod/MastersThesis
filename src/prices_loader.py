...
class StockPricesLoader:
    """
    Loads stock prices from stooq txt files, which have daily OHLCV data.
    Matches tickers from SEC filings to stooq filenames, and combines into a single parquet.
    """
    def load_stock_prices(
        ...
        for ticker in tqdm(tickers, desc="Loading Stooq files"):
            stooq_name = self._normalize_stooq_filename_ticker(ticker)
            ...
            try:
                df = self._load_stooq_file(file_path)
    ...
    def _normalize_stooq_filename_ticker(self, ticker: str) -> str:
        """
    ...
    def _load_stooq_file(self, path: Path) -> pd.DataFrame:
        """
