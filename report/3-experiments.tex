...
Overall, the experiments indicate that temporal sentiment information is most useful for classification tasks and that its effectiveness increases with aggregation level. Industry-level modeling provides the most consistent gains, followed by sector-level results, while company-level performance remains more heterogeneous. Furthermore, short to intermediate sequence lengths generally outperform longer contexts, suggesting that sentiment information decays over time. These findings support the conclusion that temporal sentiment modeling can improve directional prediction, while numerical return forecasting remains more challenging and model-dependent.